...
@section('content')
    {{-- if($role == 'siswa') return redirect()->route('lms.siswa.index'); --}}
    <div class="wrap-menu100 card mb-4">
    <div class="card-body px-5 pt-5 pb-4">
    <h3 class="mt-3 mb-2 font-weight-bold text-brand-green">Selamat datang, {{ Auth::user()->username }}!</h3>
    <span>Sebelum mulai, pilih salah satu sistem yang Anda tuju</span>
    <div class="row mt-4">

    <?php if (auth()->user()->role->name !== 'siswa') { ?>
    ...
    <?php if (auth()->user()->role->name == 'siswa' || auth()->user()->role->name == 'guru') { ?>
    <div class="col-md-4 col-12 px-1">
      <a class="wrap-element100 card mb-4 stretched-link text-decoration-none" href="{{ route('lms.' . auth()->user()->role->name . '.index') }}">
      <div class="card-body text-center">
        <i class="zmdi zmdi-balance-wallet zmdi-hc-3x"></i>
        <div class="row">
        <div class="col-12">
          <span class="text-element">LMS</span>
        </div>
        </div>
      </div>
      </a>
